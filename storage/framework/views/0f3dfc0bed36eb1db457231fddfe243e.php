...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <div class="grid-2">
        <div>
          <p class="meta">Tanggal Daftar: <?php echo e($m->created_at->format('d M Y')); ?></p>
          <div style="display:grid;grid-template-columns:140px 1fr;gap:.5rem .75rem;align-items:start">
            <div class="label">Nama</div><div><?php echo e($m->nama_mhs); ?></div>
            <div class="label">Email</div><div><?php echo e($m->email ?? '-'); ?></div>
            <div class="label">No HP</div><div><?php echo e($m->no_hp ?? '-'); ?></div>
            <div class="label">Bidang Keahlian</div><div><?php echo e(\App\Helpers\JurusanHelper::getFormat($m->jurusan ?? null)); ?></div>
            <div class="label">Jenis Kelas</div><div><?php echo e($m->jenis_kelas ?? '-'); ?></div>
            <div class="label">Asal Sekolah</div><div><?php echo e($m->asal_sekolah ?? '-'); ?></div>
            <div class="label">Status Verifikasi</div>
            <div><strong style="color:<?php echo e($m->status_verifikasi === 'verified' ? 'green' : ($m->status_verifikasi === 'rejected' ? 'red' : '#f59e0b')); ?>"><?php echo e($m->status_verifikasi === 'verified' ? 'Sudah terverifikasi' : ($m->status_verifikasi === 'rejected' ? 'Ditolak' : 'Menunggu verifikasi')); ?></strong></div>
          </div>

          <hr style="margin:1rem 0">

          <h4 style="margin:0 0 .5rem">Marketing notes</h4>
          <form method="POST" action="<?php echo e(route('marketing.pendaftar.note', $m->id)); ?>">
            <?php echo csrf_field(); ?>
            <textarea name="marketing_notes" class="notes"><?php echo e(old('marketing_notes', $m->marketing_notes)); ?></textarea>
            <div style="margin-top:.75rem;text-align:right">
              <button class="btn-save">Save notes</button>
            </div>
          </form>
          <div style="margin-top:1rem"><a href="<?php echo e(route('marketing.pendaftar.index')); ?>">← Kembali ke daftar</a></div>
        </div>
        ...
        <aside>
          ...
          <div class="doc-list">
            <?php if(!empty($m->ktp_path)): ?>
              <div class="doc-file"><div style="flex:1">KTP / Kartu Pelajar</div><a href="<?php echo e(route('marketing.pendaftar.ktp', $m->id)); ?>" class="doc-btn">Download</a></div>
            <?php endif; ?>
            <?php if(!empty($m->ijazah_path)): ?>
              <div class="doc-file"><div style="flex:1">Ijazah</div><a href="<?php echo e(route('marketing.pendaftar.ijazah', $m->id)); ?>" class="doc-btn">Download</a></div>
            <?php endif; ?>
            <?php if(!empty($m->akte_kelahiran_path)): ?>
              <div class="doc-file"><div style="flex:1">Akte Kelahiran</div><a href="<?php echo e(route('marketing.pendaftar.akte', $m->id)); ?>" class="doc-btn">Download</a></div>
            <?php endif; ?>
            <?php if(!empty($m->surat_sudah_bekerja_path)): ?>
...
</html><?php /**PATH D:\LP3IKARAWANG\LP3I_MOBILE_SERVIVE\resources\views/marketing/pendaftar/show.blade.php ENDPATH**/ ?>