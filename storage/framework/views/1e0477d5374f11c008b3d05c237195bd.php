...
<body>
  ...
  <script>
              ...
              <div class="actions">
                ...
                <div class="action-menu">
                  ...
                  <div class="menu">
                    ...
                    <div style="padding:.4rem 0;font-size:.85rem;color:#64748b;padding-left:.5rem;padding-right:.5rem;">
                      <div style="margin-bottom:.3rem;font-weight:600;">Validasi Registrasi</div>
                      <select onchange="updateRegistrationPayment(${item.id}, this.value)" style="width:100%;padding:.3rem;font-size:.85rem;margin-bottom:.3rem;cursor:pointer;border:1px solid #cbd5e1;border-radius:.25rem;">
                        <option value="">Pembayaran Registrasi</option>
          ...
          detail.innerHTML = `
            <div style="padding-top:.5rem;font-size:.95rem;color:#334155">
              ...
              <div style="margin-top:.5rem;display:flex;gap:.5rem;flex-wrap:wrap">
                ...
                <select class="action-btn" onchange="updateRegistrationPayment(${item.id}, this.value)" style="cursor:pointer;padding:0.5rem 0.75rem;">
                  <option value="">Validasi Pembayaran Registrasi</option>
                  <option value="unpaid">Belum Bayar</option>
                  <option value="paid">Sudah Bayar</option>
                </select>
                <select class="action-btn" onchange="updateRegistrationVerification(${item.id}, this.value)" style="cursor:pointer;padding:0.5rem 0.75rem;">
                  <option value="">Validasi Verifikasi Registrasi</option>
    ...
    async function updateRegistrationPayment(id, status) {
      if (!status) return;
      ...
      const fd = new FormData(); fd.append('id', id); fd.append('status', status);
      try {
        const res = await fetch('<?php echo e(route('marketing.pendaftar.registration-payment')); ?>', { method: 'POST', body: fd, headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } });
        const js = await res.json();
        if (js.success) { 
    ...
    async function updateRegistrationVerification(id, status) {
      ...
      const fd = new FormData(); fd.append('id', id); fd.append('status', status);
      try {
        const res = await fetch('<?php echo e(route('marketing.pendaftar.registration-verification')); ?>', { method: 'POST', body: fd, headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } });
        const js = await res.json();
        if (js.success) { 
...
</html><?php /**PATH D:\LP3IKARAWANG\LP3I_MOBILE_SERVIVE\resources\views/marketing/pendaftar/index.blade.php ENDPATH**/ ?>